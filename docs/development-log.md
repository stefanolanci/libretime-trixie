...
**Semantic version (one triple):** **Major.Minor.Patch** (e.g. `0.2.2`). The same triple appears in **`VERSION`**, **`setup.py`**, and the **Git tag** before `-trixie`.

| Layer | Format | Example |
|--------|--------|---------|
| **Git tag (this repo)** | `Major.Minor.Patch-trixie` (**no** leading `v` on the tag ref) | **`0.2.2-trixie`** |
| **Root `VERSION`** | `Major.Minor.Patch` + space + `trixie` | **`0.2.2 trixie`** |
| **Python `setup.py`** | `version="Major.Minor.Patch"` | **`0.2.2`** |

User-facing history for this fork is maintained **here** (no separate root changelog). [GitHub Releases](https://github.com/stefanolanci/libretime-trixie/releases) may use a readable title such as **v0.2.2-trixie** while the **tag ref** remains **`M.m.p-trixie`**. One triple ⇒ one annotated tag and one release per bump.
...
```bash
git fetch origin tag 0.2.2-trixie
git checkout 0.2.2-trixie   # detached HEAD; fine for installs
...
### 2026-05-10 — **v0.2.2-trixie** (patch)

- Bumped fork semver to **0.2.2** in **`VERSION`** and all component **`setup.py`** files.
- **Calendar fill indicators:** 24-hour shows whose filled time is reported as PostgreSQL day intervals now render as fully populated when their schedules cover the whole show.
- **Show Builder scheduling:** moving tracks within a running show preserves crossfade overlap while recalculating the remaining schedule, avoiding false multi-minute overbooking after reorder operations.
- **Git tag ref:** **`0.2.2-trixie`**. Release titles may use **v0.2.2-trixie**.

---

### 2026-05-07 — **v0.2.1-trixie** (patch)

- Bumped fork semver to **0.2.1** in **`VERSION`** and all component **`setup.py`** files.
- **Playout level detection:** low-audio detection is more reliable around quiet output and recovery conditions.
- **Fade and schedule stability:** fade playback and schedule update handling were tightened so automation state changes keep timing consistent.
- **Crossfade timing:** crossfade scheduling now uses more consistent timing calculations, including critical edge cases around transition windows.
- **Git tag ref:** **`0.2.1-trixie`**. Release titles may use **v0.2.1-trixie**.

---

### 2026-05-05 — **v0.2.0-trixie** (minor)

- Bumped fork semver to **0.2.0** in **`VERSION`** and all component **`setup.py`** files.
- **Apple Podcasts feed validation:** station feeds now expose refreshed Apple metadata consistently enough for external validators, including stable artwork URLs and cache validators for RSS/artwork refreshes.
- **Podcast artwork refresh:** dedicated podcast artwork changes now update the public feed state reliably so Apple Podcasts and validators can pick up replacements without stale metadata.
- **Episode Apple defaults:** station podcast episodes continue to emit Apple-compatible defaults, including `itunes:season` for mono-season feeds, while per-episode season/episode editing remains a future database-backed enhancement.
- **Git tag ref:** **`0.2.0-trixie`**. Release titles may use **v0.2.0-trixie**.
