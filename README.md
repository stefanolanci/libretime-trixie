...
**Distribution:** [libretime-trixie](https://github.com/stefanolanci/libretime-trixie) **v0.1.8-trixie** (release line). **Git tag on GitHub:** **`0.1.8-trixie`** (same `0.1.8` triple as `VERSION` / setuptools; repo rules disallow a `v…-trixie` ref). Mapping and rules: **[docs/development-log.md](docs/development-log.md#release-identity-and-versioning)**. Notes: [CHANGELOG.md](CHANGELOG.md).
...
- **Version label:** root `VERSION` file (e.g. `0.1.8 trixie`); `tools/version.sh` does **not** overwrite it when it already contains a semver. **Git tag (this repo):** **`Major.Minor.Patch-trixie`** without a leading `v` (see **[docs/development-log.md — Release identity](docs/development-log.md#release-identity-and-versioning)**).
