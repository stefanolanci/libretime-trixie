...
**Distribution:** [libretime-trixie](https://github.com/stefanolanci/libretime-trixie) **v0.1.14-trixie** (release line). **Git tag** (annotated checkout): **`0.1.14-trixie`** — same **`0.1.14`** triple as **`VERSION`** and Python packages; the tag ref has **no** leading **`v`** (see **[Release identity](docs/development-log.md#release-identity-and-versioning)**).
...
- **Version label:** root `VERSION` file (e.g. `0.1.14 trixie`); `tools/version.sh` does **not** overwrite it when it already contains a semver. **Git tag (this repo):** **`M.m.p-trixie`** without a leading **`v`** (e.g. **`0.1.14-trixie`**; see **[docs/development-log.md — Release identity](docs/development-log.md#release-identity-and-versioning)**).
