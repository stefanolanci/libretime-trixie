...
- **Version label:** root `VERSION` file (e.g. `0.1.8 trixie`); `tools/version.sh` does **not** overwrite it when it already contains a semver. **Git tag** = **`v` + same Major.Minor.Patch + `-trixie`** (see **[docs/development-log.md — Release identity](docs/development-log.md#release-identity-and-versioning)**).
