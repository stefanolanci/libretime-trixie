...
**Distribution:** [libretime-trixie](https://github.com/stefanolanci/libretime-trixie) **v0.1.9-trixie** (release line). **Git tag on GitHub:** **`v0.1.9-trixie`** (same `0.1.9` triple as `VERSION` / setuptools). Mapping and rules: **[docs/development-log.md](docs/development-log.md#release-identity-and-versioning)**.
...
- **Version label:** root `VERSION` file (e.g. `0.1.9 trixie`); `tools/version.sh` does **not** overwrite it when it already contains a semver. **Git tag (this repo):** **`vM.m.p-trixie`** (e.g. **`v0.1.9-trixie`**; see **[docs/development-log.md — Release identity](docs/development-log.md#release-identity-and-versioning)**).
