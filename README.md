...
**Distribution:** [libretime-trixie](https://github.com/stefanolanci/libretime-trixie) **v0.2.2-trixie** (release line). **Git tag** (annotated checkout): **`0.2.2-trixie`** — same **`0.2.2`** triple as **`VERSION`** and Python packages; the tag ref has **no** leading **`v`** (see **[Release identity](docs/development-log.md#release-identity-and-versioning)**).
...
- **Icecast listener startup burst moderated:** `installer/icecast/icecast.xml` enables a conservative `burst-on-connect=1` / `burst-size=32768` default to shorten player startup without the larger repeated slices seen with 64k bursts during reconnect loops.
...
- **Version label:** root `VERSION` file (e.g. `0.2.2 trixie`); `tools/version.sh` does **not** overwrite it when it already contains a semver. **Git tag (this repo):** **`M.m.p-trixie`** without a leading **`v`** (e.g. **`0.2.2-trixie`**; see **[docs/development-log.md — Release identity](docs/development-log.md#release-identity-and-versioning)**).
