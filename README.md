...
**Distribution:** [libretime-trixie](https://github.com/stefanolanci/libretime-trixie) **v0.2.0-trixie** (release line). **Git tag** (annotated checkout): **`0.2.0-trixie`** — same **`0.2.0`** triple as **`VERSION`** and Python packages; the tag ref has **no** leading **`v`** (see **[Release identity](docs/development-log.md#release-identity-and-versioning)**).
...
- **Version label:** root `VERSION` file (e.g. `0.2.0 trixie`); `tools/version.sh` does **not** overwrite it when it already contains a semver. **Git tag (this repo):** **`M.m.p-trixie`** without a leading **`v`** (e.g. **`0.2.0-trixie`**; see **[docs/development-log.md — Release identity](docs/development-log.md#release-identity-and-versioning)**).
