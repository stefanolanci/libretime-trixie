...
from ..liquidsoap.utils import escape_liquidsoap_annotate_value
...
def create_liquidsoap_annotation(file_event: FileEvent) -> str:
    ...
    if fade_in_s != file_event.fade_in / 1000.0 or fade_out_s != file_event.fade_out / 1000.0:
        logger.warning(
            "clamped liq fade for media id=%s: fade_in %s->%ss fade_out %s->%ss",
            file_event.id,
            file_event.fade_in / 1000.0,
            fade_in_s,
    ...
    # Use a dedicated metadata key so amplify() does not pick up stray "replay_gain"
    # from ICY, decoder pre-roll, or other map_metadata (LS 2.3 first-track ~−91 dBFS).
    if file_event.replay_gain is not None:
        annotations["libretime_replay_gain"] = f"{file_event.replay_gain} dB"
    ...
    if file_event.artist_name:
        annotations["artist"] = file_event.artist_name.strip()

    if file_event.track_title:
        annotations["title"] = file_event.track_title.strip()

    def format_annotation(key: str, value) -> str:
        if isinstance(value, str):
            return f'{key}="{escape_liquidsoap_annotate_value(value)}"'
        return f'{key}="{value}"'

    annotations_str = ",".join(
        format_annotation(key, value) for key, value in annotations.items()
    )

    path = file_event.local_filepath.expanduser().resolve().as_posix()
    return "annotate:" + annotations_str + ":" + path
...
class TelnetLiquidsoap:
    ...
    def switch_source(self, sourcename, status):
        ...
        try:
            ...
            streaming = status == "on"
            self.liq_client.source_switch_status(sourcename, streaming)
        except OSError as exception:
            logger.exception(exception)
...
class Liquidsoap:
    ...
    def verify_correct_present_media(self, scheduled_now: List[AnyEvent]) -> None:
        ...
        if to_be_removed:
            ...
            slot_map = {
                q: (it.row_id, it.id)
                for q, it in self.liq_queue_tracker.items()
                if it is not None
            }
            logger.info("Liquidsoap queue slots before remove (queue_id -> row_id, file_id): %s", slot_map)
