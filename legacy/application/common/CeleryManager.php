...
class CeleryManager
{
    ...
    {
        // Celery 5+ no longer supports the AMQP result backend. The worker uses Redis
        // (default redis://127.0.0.1:6379/0); results are stored under celery-task-meta-<id>.
        $taskId = $task->getDbTaskId();
        if ($taskId === '' || $taskId === null) {
            return false;
        }
        $key = 'celery-task-meta-' . $taskId;
        $host = getenv('LIBRETIME_REDIS_HOST') ?: '127.0.0.1';
        $port = (int) (getenv('LIBRETIME_REDIS_PORT') ?: 6379);
        $database = (int) (getenv('LIBRETIME_REDIS_DB') ?: 0);
        $password = getenv('LIBRETIME_REDIS_PASSWORD');
        $params = [
            'scheme' => 'tcp',
            'host' => $host,
            'port' => $port,
