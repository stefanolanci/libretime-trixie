...
SQL;
    ...
    /**
     * Returns instance IDs (keys) for shows with an automatic playlist, for calendar rendering.
     *
     * Uses the same overlap rule as getContentCount() and getIsFull():
     * starts < p_end AND ends > p_start. The previous strict-inner filter
     * (starts > p_start AND ends < p_end) omitted instances that touch the visible
     * range edges, so week view showed "empty" for Monday 00:00 / Sunday→midnight blocks
     * despite has_autoplaylist and no rows in cc_schedule yet.
     *
     * @see https://github.com/libretime/libretime/issues/3235
     */
    public static function getShowHasAutoplaylist($p_start, $p_end)
    {
        ...
        try {
            $showInstances = CcShowInstancesQuery::create()
                ->filterByDbStarts($p_end->format(DEFAULT_TIMESTAMP_FORMAT), Criteria::LESS_THAN)
                ->filterByDbEnds($p_start->format(DEFAULT_TIMESTAMP_FORMAT), Criteria::GREATER_THAN)
                ->leftJoinCcShow()
