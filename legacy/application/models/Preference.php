...
class Application_Model_Preference
{
        ...
        try {
            ...
            if ($result == 0) {
                ...
                if ($isUserValue) {
                    ...
                    // Must match the COUNT branch above, otherwise we read the wrong row when
                    // duplicate keystr rows exist (e.g. legacy user + station prefs).
                    $sql .= ' AND subjid IS NULL';
                }
    ...
    public static function SetStationLogo($imagePath)
    {
        ...
        if ($image === false) {
            Logging::warn('Could not read station logo file: ' . $imagePath);

            return;
        }
        self::setValue('logoImage', base64_encode($image));
    }
    ...
    {
        self::setValue('auto_transition', self::normalizeBoolPrefForStorage($value), false);
    }

    public static function GetAutoTransition()
    {
        return self::normalizeBoolPrefForCheckbox(self::getValue('auto_transition', false, true));
    }

    public static function SetAutoSwitch($value)
    {
        self::setValue('auto_switch', self::normalizeBoolPrefForStorage($value), false);
    }

    public static function GetAutoSwitch()
    {
        return self::normalizeBoolPrefForCheckbox(self::getValue('auto_switch', false, true));
    }

    /** @param mixed $value */
    private static function normalizeBoolPrefForStorage($value)
    {
        if ($value === '1' || $value === 1 || $value === true) {
            return '1';
        }
        if (is_string($value)) {
            $l = strtolower(trim($value));

            return ($l === 'true' || $l === 'on' || $l === 'yes') ? '1' : '0';
        }
    ...
    /** @param mixed $stored */
    private static function normalizeBoolPrefForCheckbox($stored)
    {
        if ($stored === '1' || $stored === 1 || $stored === true) {
            return '1';
        }
        if (is_string($stored)) {
            $l = strtolower(trim($stored));
