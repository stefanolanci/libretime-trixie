...
class ApiController extends Zend_Controller_Action
{
    ...
    {
        // Always serve now-playing for the bundled /embed/player and public widgets.
        // This action is already excluded from API-key auth in init(); gating it again on
        // "third party API" breaks the station homepage when that preference is off.
        $this->view->layout()->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
    ...
    public function updateSourceStatusAction()
    {
        ...
        // Persist connection and switch state before notifying playout so GET /api/v2/stream/state
        // sees a consistent live_dj / live_dj_switch when switch_source is handled (avoids race).
